...
RSpec.describe CommentsController, type: :controller do
	describe "comments#create action" do
		it "should allow users to create comments on grams" do
			...
			post :create, params: { gram_id: gram.id, comment: { message: 'Nice picture!' } }
			expect(response).to redirect_to root_path
			expect(gram.comments.length).to eq 1
			expect(gram.comments.first.message).to eq 'Nice picture!'
		end
		...
		it "should return http status code of not found if the gram isn't found" do
			user = FactoryBot.create(:user)
			sign_in user
			post :create, params: { gram_id: 'NONEXISTANTGRAM', comment: { message: 'Nice gram!' } }
